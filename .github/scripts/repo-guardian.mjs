...
async function ensureLabels() {
  ...
  for (const [key, label] of Object.entries(LABELS)) {
    try {
      await gh(`label create "${label}" --repo ${REPO} --color "E1E400" --description "Repo Guardian automated label" 2>/dev/null || true`);
    } catch (error) {
...
async function getOpenPRs() {
  const data = await gh(`pr list --repo ${REPO} --state open --json number,headRefName,baseRefName,author,isDraft,mergeable,mergeStateStatus,createdAt,labels,autoMergeRequest,title,url`);
  return JSON.parse(data);
}

async function getPRChecks(prNumber) {
  const data = await gh(`pr checks ${prNumber} --repo ${REPO} --json name,status,conclusion,link,detailsUrl,startedAt || echo "[]"`);
  return JSON.parse(data);
}

async function getFailedWorkflowRuns(prNumber) {
  try {
    const data = await gh(`api repos/${REPO}/pulls/${prNumber} --jq '.head.sha'`);
    const headSha = data.trim();
    
    const runs = await gh(`api repos/${REPO}/commits/${headSha}/check-runs --jq '.check_runs[] | select(.conclusion == "failure" or .conclusion == "cancelled") | {id, name, conclusion}'`);
    ...
    console.error(`Error getting failed runs for PR #${prNumber}:`, error.message);
    return [];
  }
}
...
async function updatePRBranch(prNumber) {
  try {
    ...
    try {
      await gh(`api --method PUT repos/${REPO}/pulls/${prNumber}/update-branch`);
      return true;
    } catch (error2) {
...
async function enableAutoMerge(prNumber) {
  try {
    ...
    console.error(`  Failed to enable auto-merge for PR #${prNumber}:`, error.message);
    return false;
  }
}
...
async function addLabel(prNumber, label) {
  try {
    ...
    console.error(`  Failed to add label ${label} to PR #${prNumber}:`, error.message);
  }
}
...
async function isBehindBase(prNumber) {
  try {
    const result = await gh(`pr view ${prNumber} --repo ${REPO} --json mergeStateStatus`);
    const data = JSON.parse(result);
    return data.mergeStateStatus === 'BEHIND' || data.mergeStateStatus === 'DIRTY';
  } catch (error) {
    return false;
  }
}
...
async function handleSecretViolation(pr, checks) {
  const secretChecks = checks.filter(c => 
  ...
  if (secretChecks.length > 0 && !await hasLabel(pr, LABELS.SECRET_VIOLATION)) {
...
async function handleStalepr(pr) {
  ...
  if (daysSinceCreated > CONFIG.STALE_DAYS && !await hasLabel(pr, LABELS.STALE)) {
    console.log(`  📅 Marking PR #${pr.number} as stale (${Math.floor(daysSinceCreated)} days old)`);
...
async function processPR(pr) {
  ...
  if (behind && !await hasLabel(pr, LABELS.AUTO_UPDATED)) {
    ...
    if (updated) {
      ...
      await addComment(pr.number, '🔄 **Auto-Update**: This PR has been automatically updated with the latest changes from the base branch.');
      
      // Remove the label after 1 hour so it can be updated again if needed
      setTimeout(() => removeLabel(pr.number, LABELS.AUTO_UPDATED), 60 * 60 * 1000);
    }
  }
  ...
  if (failedChecks.length > 0 && !await hasLabel(pr, LABELS.RERUN_ATTEMPTED)) {
    console.log(`  Found ${failedChecks.length} failed checks, attempting rerun...`);
    ...
    if (rerunCount > 0) {
      ...
      await addComment(pr.number, `🔄 **Auto-Retry**: Automatically rerunning ${rerunCount} failed check(s). This is a one-time retry.`);
      
      // Remove the label after 2 hours so it can retry again later if needed
      setTimeout(() => removeLabel(pr.number, LABELS.RERUN_ATTEMPTED), 2 * 60 * 60 * 1000);
    }
  }

  // Enable auto-merge if not already enabled and checks are passing
  if (!pr.autoMergeRequest && pr.mergeable === 'MERGEABLE' && 
      checks.every(c => c.conclusion === 'success' || c.status === 'in_progress')) {
